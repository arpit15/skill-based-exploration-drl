...
from HER.cdq.cdq import CDQ
from HER.cdq.util import normal_mean, normal_std, mpi_max, mpi_sum
import HER.common.tf_util as U
from HER.cdq.util import read_checkpoint_local
...
def train(env, nb_epochs, nb_epoch_cycles, render_eval, reward_scale, render, param_noise, actor, critic, additional_critic,
    ...
    agent = CDQ(actor, critic, additional_critic, memory, env.observation_space.shape, env.action_space.shape,
        ...
        for epoch in range(global_t, nb_epochs):
            ...
            if dologging and rank==0: 
                ...
                if rank==0  and (epoch+1)%save_freq == 0 :
                    ...
                    saver.save(U.get_session(), logdir+"/model/cdq", global_step = epoch)
