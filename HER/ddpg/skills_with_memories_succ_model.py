...
class DDPGSkill(object):
    '''skill with successor model next state prediction '''
    def __init__(self, observation_shape=(1,), normalize_observations=True, observation_range=(-5., 5.), 
        ...
        print("searching for successor model in %s"%osp.join(self.restore_path, 'succ_model'))
        self.succ_model = regressor(in_shape = observation_shape[0], 
                            out_shape = observation_shape[0]-3,
                            name = "%s/succmodel"%skill_name, sess=None,
                            log_dir=None,
                            whiten_data = None, train = False, in_tensor = self.obs0)
        ...
        self.loader_successor_model = tf.train.Saver(self.create_restore_var_dict_successor_model(model_name='suc_pred_model'))
        self.loader_successor_prediction_model = tf.train.Saver(self.create_restore_var_dict_successor_model(model_name="succmodel"))

    def create_restore_var_dict_successor_model(self, model_name):
        var_restore_dict_successor_model = {}

        model_var = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='%s/%s'%(self.skill_name, model_name))
        ...
        if MPI.COMM_WORLD.Get_rank() == 0:
            logger.debug("restoring following %s model vars\n"%(model_name)+"-"*20)
    ...
    def restore_skill(self, path, sess):
        ...
        model_checkpoint_path = read_checkpoint_local(osp.join(path, "succ_model"))
        if model_checkpoint_path:
            self.loader_successor_prediction_model.restore(U.get_session(), model_checkpoint_path)
            if MPI.COMM_WORLD.Get_rank() == 0: logger.info("Successfully loaded successor model for %s skill"%self.skill_name)
    ...
    def get_terminal_state_from_memory(self, obs, primitive_params):
        ...
        next_obs_nn = self.ending_state[min_dist_idx].copy()
