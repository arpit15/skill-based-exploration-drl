...
class BaxterEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    """cts env, 6dim
    state space: relative state space position of gripper, (block-gripper), angle bw gripper, block & target and (target-block)
    random restarts for block and target on the table
    reward function: - 1(not reaching)
    actions: (delta_x, delta_y) 5cm push
    starting state: (0.63, 0.2, 0.59, 0.27, _ , 0.55, 0.3)
    max_num_steps = 50
    """
    def __init__(self):
    ...
    def _get_obs(self):
        ...
        angle_bw_block_gripper = atan2(box_y - ee_y, box_x - ee_x)
