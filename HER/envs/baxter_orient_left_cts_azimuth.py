...
class BaxterEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    """cts env, 6dim
    state space: relative state space position of gripper, (block-gripper), azimuth and (target-block)
    random restarts for block and target on the table
    reward function: - 1(not reaching)
    actions: (delta_x, delta_y) 5cm push
    starting state: (0.63, 0.2, 0.59, 0.27, 0, 0.55, 0.3)
    max_num_steps = 50
    """
    def __init__(self):
    ...
    def _get_obs(self):
        ...
        _, _, box_orient_z = euler_from_matrix(self.data.site_xmat[1].reshape((3,3)), 'rxyz')

        target_x, target_y = self.data.site_xpos[2][:2]

        state = np.array([ee_x, ee_y, box_x, box_y, box_orient_z, target_x, target_y])
