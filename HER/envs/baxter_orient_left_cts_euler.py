...
class BaxterEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def _get_obs(self):
        ...
        box_orient_x, box_orient_y, box_orient_z = euler_from_matrix(self.data.site_xmat[1].reshape((3,3)), 'rxyz')

        target_x, target_y = self.data.site_xpos[2][:2]

        state = np.array([ee_x, ee_y, box_x, box_y, box_orient_x, box_orient_y, box_orient_z, target_x, target_y])
