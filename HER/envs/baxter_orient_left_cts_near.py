...
class BaxterEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, max_len=50):
        ...
        ## starting pose
        # print(self.data.qpos[-2:], self.data.qpos[8:10])
    ...
    def reset_model(self):
        ...
        ## random box location
        r = np.random.uniform(low=0.051, high=0.1, size=1)
        theta = np.random.uniform(low=-np.pi, high=np.pi, size=1)
        qpos[8] = 0.55 + qpos[-2] + r*np.cos(theta)
        qpos[9] = 0.3 + qpos[-1] + r*np.sin(theta)

        self.set_state(qpos, qvel)

        ## random gripper location
        r = np.random.uniform(low=0.031, high=0.08, size=1)
        theta2 = np.random.uniform(low=-np.pi/3, high=np.pi/3, size=1)
        # if(np.sign(theta)!=np.sign(theta2)):
        #     theta2 *= -1
        # print((theta), (theta2))

        target_pos = np.array([float(qpos[8] + r*np.cos(theta2 + theta)) , float(qpos[9] + r*np.sin(theta2+ theta)) , 0.15])
