...
class BaxterEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def __init__(self, max_len=50, near_init = False, fixed_obj_init = False):
        ...
        print(dirname)
        mujoco_env.MujocoEnv.__init__(self, os.path.join(dirname, "mjc/baxter_orient_left_cts_rotate.xml") , 1)
    ...
    def reset_model(self):
        ...
        if self.near_init:
            ...
            while 1:
                qpos = self.init_qpos + self.np_random.uniform(low=-.005, high=.005, size=self.model.model.nq)
                qvel = self.init_qvel + self.np_random.uniform(low=-.005, high=.005, size=self.model.model.nv)
                
                ## random target location
                qpos[-2:] = qpos[-2:] + self.np_random.uniform(low=-box_size, high=box_size, size=2)
                ## random box location
...
if __name__ == "__main__":
    ...
    env = BaxterEnv(max_len, near_init, fixed_obj_init)
    EVAL_EPISODE = 100
    reward_mat = []
