...
class CDQ(object):
    def __init__(self, actor, critic, additional_critic, memory, observation_shape, action_shape, param_noise=None, action_noise=None,
        ...
        if self.select_action:
            ...
            Q_obs1_1 = denormalize(target_critic1(normalized_obs1, choose_actions(target_actor_prediction_next_state_tf, skillset, self.W_select)), self.ret_rms)
        
        else:
            Q_obs1_0 = denormalize(target_critic(normalized_obs1, target_actor_prediction_next_state_tf), self.ret_rms)
            Q_obs1_1 = denormalize(target_critic1(normalized_obs1, target_actor_prediction_next_state_tf), self.ret_rms)
    ...
    def setup_critic_optimizer(self):
        ...
        if self.critic_l2_reg > 0.:
            critic_reg_vars = [var for var in (self.critic.trainable_vars + self.critic1.trainable_vars) if 'kernel' in var.name and 'output' not in var.name]
        ...
        critic_shapes = [var.get_shape().as_list() for var in (self.critic.trainable_vars + self.critic1.trainable_vars)]
    ...
    def train(self, summary_var, temperature = 1., update_actor = True):
        ...
        if update_actor: self.actor_optimizer.update(actor_grads, stepsize=self.actor_lr)
