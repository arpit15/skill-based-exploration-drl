...
from HER.planning.MC_planning_with_memories_agent import Planning_with_memories
            ...
            for cycle in range(nb_epoch_cycles):
                ...
                for t_rollout in range(int(nb_rollout_steps/MPI.COMM_WORLD.Get_size())):
                    ...
                    if kwargs['look_ahead'] and (np.random.rand() < exploration.value(epoch*nb_epoch_cycles + cycle)):
                        # start = time.time()
                        paction, planner_info = look_ahead_planner.create_plan(state=obs, agent=agent)
