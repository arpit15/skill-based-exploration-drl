...
class Planning_with_memories:
    ...
    def create_plan(self, env, state, height = None):
        ...
        info['prob'] = curr_node.child.prob
        info['next_state_nn'] = self.skillset.skillset[chosen_skill].ending_state[curr_node.child.id_in_memory]
        
        target_tmp = state[-3:]
        next_state_nn_tmp = np.concatenate((info['next_state_nn'], target_tmp))
        info['next_state_nn'] = self.skillset.skillset[chosen_skill].get_full_state(next_state_nn_tmp, prev_obs=state)

        # print("state", state[:6])
        # print("next state nn", next_state_nn[:6])
