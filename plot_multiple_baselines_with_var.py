...
for agent_name, ext, color in zip(agent_name_list, extensions, color_list):
	...
	for j in range(1,NUM_RUNS+1):
		curr_data = pd.read_csv("/Users/virtualworld/new_RL3/corl_paper_results/clusters-v1/%s%s-%s/run%d/progress.csv"%(env_name[0], ext, env_name[1], j)).fillna(0.)["eval/success"]
